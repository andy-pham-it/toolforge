...
const CATALOG = [
    ...
    {
        name: '@andy-toolforge/authoring',
        version: '0.1.3',
        description: 'Lesson authoring — generate lessons, scaffold series, embed images into markdown',
        useCases: [
            'Generate structured lesson plans from topic + audience via LLM',
            'Scaffold a series directory with TOC, numbered lessons, and images/',
            'Replace image placeholders in markdown with generated images via Gemini Images',
            'Validate series structure, internal links, and image references',
        ],
        keyExports: [
            { name: 'generateLesson', description: 'Generate lesson plans via LLM' },
            { name: 'scaffoldSeries', description: 'Scaffold series dir (TOC + lessons + images/)' },
            { name: 'embedImagesToMarkdown', description: 'Replace image placeholders via Gemini Images' },
            { name: 'validateSeries', description: 'Check series structure, links, images' },
        ],
        dependencies: ['@andy-toolforge/core', '@google/genai'],
        skillFiles: false,
    },
    {
        ...
        useCases: [
            ...
            'Run opencode sessions and auto-commit changes from an MCP client',
            'Bridge opencode CLI with messaging platforms via Hermes',
        ],
        keyExports: [
            { name: 'createServer', description: 'MCP server bridging opencode CLI' },
    ...
    },
    {
        ...
        version: '0.1.1',
        description: 'LLM gateway pipeline — stages, adapters, metrics, CLI, and HTTP',
        useCases: [
            'Build LLM pipelines with modular stages (pre/post-process, routing, caching)',
            'Collect request/token/cost metrics per stage',
            'Expose a gateway over CLI or HTTP',
        ],
        keyExports: [
            { name: 'createGateway', description: 'Factory requiring createAdapter callback' },
            { name: 'Gateway', description: 'Pipeline orchestrator (stages + adapters)' },
            { name: 'MetricsCollector', description: 'Request/token/cost metrics' },
        ],
        dependencies: ['@andy-toolforge/core', 'express'],
        skillFiles: true,
        skillPrefix: 'llm-gateway',
    },
    {
        name: '@andy-toolforge/llm-gateway-core',
        version: '0.1.1',
        description: 'LLM gateway pipeline stage primitives (TypeScript)',
        useCases: [
            'Build reusable pipeline stage chains with typed middleware',
            'Compose async execute(context, next) stages',
            'TypeScript foundation for llm-gateway extensions',
        ],
        keyExports: [
            { name: 'createPipeline', description: 'Build stage chains (TypeScript)' },
            { name: 'Pipeline', description: 'async execute(context, next) chain' },
            { name: 'Stage', description: 'Stage primitive for pipeline chains' },
        ],
        dependencies: [],
        skillFiles: false,
    },
    {
        ...
        version: '0.5.0',
        description: 'SDLC workflow templates, standards, and validation MCP server',
        useCases: [
            'Fetch PRD/BRD/runbook templates and industry standards',
            'Render templates with context variables ({{ var }}, {% if %}, {% for %})',
            'Validate skills and SDLC documents against standards (agile, IEEE, ISO, arc42)',
            'Search SDLC skills by keywords or triggers',
        ],
        keyExports: [
            { name: 'mcpServer', description: 'MCP server for SDLC templates/skills' },
            { name: 'sdlcListTemplates', description: 'List templates/standards by category' },
            { name: 'sdlcGetTemplate', description: 'Read a template file by ID' },
            { name: 'sdlcRenderTemplate', description: 'Render a template with context variables' },
            { name: 'sdlcValidateSkill', description: 'Validate a SKILL.md against a test case' },
            { name: 'sdlcGetStandard', description: 'Read a standard/reference file by ID' },
        ],
        dependencies: ['@andy-toolforge/core', 'glob', 'js-yaml'],
        skillFiles: true,
        skillPrefix: 'sdlc-workflows',
    },
    {
        name: '@andy-toolforge/db-mongo',
        version: '0.1.0',
        description: 'Generic MongoDB wrapper: connection, collections, migrations',
        useCases: [
            'Connect to MongoDB with pooling and timeouts',
            'Run ordered schema migrations',
        ],
        keyExports: [
            { name: 'MongoDatabase', description: 'MongoDB connection wrapper (connect/close/collection/ping)' },
            { name: 'MigrationRunner', description: 'Ordered migration runner with applied-tracking' },
        ],
        dependencies: ['mongodb'],
        skillFiles: false,
    },
    {
        name: '@andy-toolforge/cli',
        version: '0.1.0',
        description: 'Zero-dependency CLI toolkit: arg parsing, spinner, config loading',
        useCases: [
            'Parse CLI arguments with flags and positionals',
            'Show progress spinners in terminal',
            'Load JSON/JS config with env overrides',
        ],
        keyExports: [
            { name: 'parseArgs', description: 'Parse argv into flags/positionals/values' },
            { name: 'Spinner', description: 'Terminal spinner with injectable stream' },
            { name: 'loadConfig', description: 'Load config files with defaults + env override' },
        ],
        dependencies: [],
        skillFiles: false,
    },
    {
        name: '@andy-toolforge/reporting',
        version: '0.1.0',
        description: 'Markdown/HTML/PDF report generation',
        useCases: [
        ...
        keyExports: [
            { name: 'toMarkdown', description: 'Structured data -> markdown' },
            { name: 'toHTML', description: 'Markdown -> HTML' },
            { name: 'toPDF', description: 'Markdown -> PDF buffer' },
        ],
        dependencies: ['pdfkit'],
        skillFiles: false,
    },
